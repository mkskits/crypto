...
def main():
    ...
    dt_pd_twitter['twitter_vol_30d_ann'] = dt_pd_twitter['twitter_log_ret'].rolling(window=30, center=False).std() * np.sqrt(365)
    ...
    ax1.plot(dt_pd_twitter[['tweets']], color='blue', label='Number of Tweets')
    ax1.legend(loc='upper left')
    # ax1b = ax1.twinx()
    # ax1b.plot(dt_pd_fin[['Global.Govt']], color='red', label='GA Treasuries')
    # ax1b.legend(loc='upper right')
    ax2.plot(dt_pd_twitter[['twitter_vol_30d_ann']], color='blue', label='$\sigma_{Tweets}$')
    ...
    plt.savefig('pt_twitter_std.pdf')
