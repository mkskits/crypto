...
def main():
    ...
    dt_pd_financial['price'] = np.log(100 * dt_pd_financial['price'] / \
                                dt_pd_financial.iloc[0]['price'])

    dt_pd_financial['tweets'] = np.log(100 * dt_pd_financial['tweets'] / \
                                 dt_pd_financial.iloc[0]['tweets'])

    dt_pd_financial['wikipedia'] = np.log(100 * dt_pd_financial['wikipedia'] / \
                                dt_pd_financial.iloc[0]['wikipedia'])

    dt_pd_financial['xau'] = 100 * dt_pd_financial['xau'] / \
    ...
    ax2.plot(dt_pd_financial[['wikipedia']], color='darkslategrey', label='Wikipedia', alpha=0.9, ls = ':')
