...
def main():
    ...
    dt_pd_google_daily = pd.read_pickle('dt_pd_google_btc_daily.pickle')
    # dt_pd_google_daily = dt_pd_google_daily.drop('segment', 1)
    dt_pd_google_monthly = pd.read_pickle('dt_pd_google_btc_monthly.pickle')
    dt_pd_google_daily.rename(columns={'google_tr_btc': 'google'}, inplace=True)
    dt_pd_google_monthly.rename(columns={'google_tr_btc': 'google'}, inplace=True)
    ...
    dt_pd_google_daily.index.name = ''
    dt_pd_google_monthly.index.name = ''

    f, (ax1) = plt.subplots(1, 1, sharex=True, sharey=True)
    ax1.plot(dt_pd_google_monthly[['google']], color = 'blue', label='Google Trends (Monthly)', alpha = 1, ls = '--')
    ax1.plot(dt_pd_google_daily[['google']], color = 'red', label='Google Trends (Daily)', alpha = 1)
    ...
    plt.savefig('pt_google_comparison.pdf')
