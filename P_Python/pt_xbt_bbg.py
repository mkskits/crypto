...
def main():
    ...
    dt_pd_wiki = pd.read_pickle('dt_pd_xbt_bbg.pickle')

    top = plt.subplot2grid((4,4), (0,0), rowspan=3, colspan=4)
    top.plot(dt_pd_wiki.index, dt_pd_wiki['price_usd'])
    top.plot(dt_pd_wiki.index, dt_pd_wiki['price_usd_MAVG30'])
    top.legend()
    # plt.gcf().set_size_inches(8, 8)
    plt.title('Bitcoin Price in USD (daily) and First Differences')
    bottom = plt.subplot2grid((4,4), (3,0), rowspan=1, colspan=4)
    bottom.bar(dt_pd_wiki.index, dt_pd_wiki['price_usd_fd'])
