...
# Social Feedback Cycle
  ...
  # information sharing (impulse: search - respone: sharing)
    ir_sharing <- irf(fit, impulse = c('google.log.rtn'), response = c('tweets.log.rtn'),
               ortho = T, n.ahead=12, boot=T, ci=0.95, runs = 100) # basic IR functions
    while (!is.null(dev.list()))  dev.off()
    # ir1$irf$wikipedia.log.rtn = 100 * ir1$irf$wikipedia.log.rtn
    pdf('../F_Figs/pt_fit_baseline_b_sharing.pdf')
  ...
  # Price (impulse: sharing - respone: price)
    ...
    while (!is.null(dev.list()))  dev.off()
    # ir1$irf$wikipedia.log.rtn = 100 * ir1$irf$wikipedia.log.rtn
    pdf('../F_Figs/pt_fit_baseline_b_price_s.pdf')
...
# User adoption Cycle  
  # Adoption (impulse: search - respone: users)
    ir_adoption <- irf(fit, impulse = c('google.log.rtn'), response = c('new_users.log.rtn'),
                    ortho = T, n.ahead=12, boot=T, ci=0.95, runs = 100) # basic IR functions
    while (!is.null(dev.list()))  dev.off()
    # ir1$irf$wikipedia.log.rtn = 100 * ir1$irf$wikipedia.log.rtn
    pdf('../F_Figs/pt_fit_baseline_b_adoption.pdf')
  ...
  #  Price (impulse: seach - respone: price)
    ir_price_d <- irf(fit, impulse = c('google.log.rtn'), response = c('price.log.rtn'),
                      ortho = T, n.ahead=12, boot=T, ci=0.95, runs = 100) # basic IR functions
    while (!is.null(dev.list()))  dev.off()
    # ir1$irf$wikipedia.log.rtn = 100 * ir1$irf$wikipedia.log.rtn
    pdf('../F_Figs/pt_fit_baseline_b_price_d.pdf')
