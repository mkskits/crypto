...
# adf test (need to convert xts object to ts in order to apply adf test)
  google.abs <- as.ts(dt.google$google)
  google.abs <- na.remove(google.abs)
  print(adf.test(google.abs, k = 27))
   
  google.fd <- as.ts(dt.google$first.differences)
  google.fd <- na.remove(google.fd)
  print(adf.test(google.fd, k = 27))
  
  google.log.return <- as.ts(dt.google$log.return)
  google.log.return <- na.remove(google.log.return)
  print(adf.test(google.log.return, k = 27))
