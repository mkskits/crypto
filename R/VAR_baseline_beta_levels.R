...
# data input 
  ...
  xts.data$price.log.rtn <- log(xts.data$price)
  # xts.data$google.log.rtn <- diff(log(xts.data$google),lag = 1)
  xts.data$wikipedia.log.rtn <- log(xts.data$wikipedia)
  xts.data$tweets.log.rtn <- log(xts.data$tweets)
  xts.data$new_users.log.rtn <- log(xts.data$new_users)
  # addition to baseline_a
